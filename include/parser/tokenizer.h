 ...
 *
 * This enum is an extensive list of all possible types of tokens of the given programming language, plus several
 * "meta-tokens" such as EOL, denting symbols etc.
 */
enum class TokenType {
    // Keywords
    IMPORT,
 ...
 *
 * Provides minimal information about the token, information being the type of token and its representation aka lexeme
 * (if applicable and not redundant; for example, representation is obviously not applicable to an indent).
 */
class Token {
public:
    /** @brief Constructs an empty token, only used for the first token of the file.
     */
    Token();

    /**
     * @brief Constructs a token from its type if either the token has no representation or it is redundant.
     */
...
public:
    /**
     * @brief Construct tokenizer entity from `std::istream` object (from where to read) and a meta parameter
     * `spaces_per_tab`.
     */
     ...
     *
     * Reads a new token from the stream (if possible) and stores it in `current_token_`. If provided with `expected`
     * parameter, checks whether the token just read is of same type. If not, throws an error.
     */
